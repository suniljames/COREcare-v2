...
| `rls_bypass_by_design` | `true` for `is_superuser`-gated cross-tenant routes (the View-As emergency kill switch); v1 has no separate platform-operator role per #236 |
...
### dashboard
_family portal landing, per-client detail, billing PDF, health-report PDF; mounted at `/family/`_
