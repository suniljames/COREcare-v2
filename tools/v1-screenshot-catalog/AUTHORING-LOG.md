...
| 2026-05-08 | 20:00–20:50 | family-member | 001, 002, 003, 004 | `03b4148003d67bc8c98129f1d1a8e3bf8f5935d367d5d8b27776bf9ef3afdf92` | suniljames | **Inventory-bug discovery**: all 4 family-member WebPs are Django `DEBUG=True` 404 pages. The inventory documents these routes as `/family/...` but `dashboard.urls` is included under `/dashboard/` (`elitecare/urls.py:186`), so the real paths are `/dashboard/family/...`. The `/family/` redirect at `elitecare/urls.py:57–59` reaches the named pattern via reverse, but the literal `/family/dashboard/` etc. paths return 404. The crawler hit the inventory-documented paths and captured 404s. Captions describe the captured 404 + the actual route + the intended UI behavior (derived from `dashboard/views.py:159` `family_dashboard`, `:180` `family_client_detail`, `:314` `family_billing_pdf`, `:459` `family_health_report_download`). **Inventory bug should be fixed** — file follow-up issue to either (a) update inventory paths to `/dashboard/family/...` and re-crawl, or (b) add URL-rewrite middleware so `/family/...` resolves to the real routes. |
